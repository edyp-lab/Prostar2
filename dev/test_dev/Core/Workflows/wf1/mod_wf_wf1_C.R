...
#' 
#' 
mod_wf_wf1_C_server <- function(id, dataIn=NULL, remoteReset=FALSE, forcePosition = NULL){
  moduleServer(
    id,
    function(input, output, session){
      ...
      observeEvent(req(forcePosition()), {
        print(paste0("MODULE C : New value for forcePosition (envoi à MODULE TL_ENGINE : ", forcePosition()))
      })
      
      # Catch the reset events (local or remote)
      observeEvent(req(c(rv$tmp_engine$reset()!=0, remoteReset()!=0)), { 
        print(paste0('MODULE C : new value for rv$tmp_engine$reset() = ', rv$tmp_engine$reset()))
        print(paste0('MODULE C : new value for remoteReset() = ', remoteReset()))
        UpdateDataIn()
        
        #print("MODULE C : Reset all screens inputs")
