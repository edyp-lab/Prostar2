...
mod_wf_wf1_B_server <- function(id, dataIn=NULL, indice=NULL){
  moduleServer(
    id,
    function(input, output, session){
      ...
      session$userData$mod_B_obs_2 <-  observeEvent(input$btn_valid,{
        ...
        name <- paste0('proc B from i =', rv$indice)
        ...
        rv$dataIn <- append(rv$dataIn, setNames(10 * rv$dataIn[[rv$indice]], name))
