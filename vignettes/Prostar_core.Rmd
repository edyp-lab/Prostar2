...
## General behaviour of module processes

This is a list of how the modules of data processing work:

  * Each process module has got a 'Reset' button which allows to set all inputs and plots to their default value. If the process has several steps, the screen returns to the first one.
  * Each process module has a 'Validate' button which allows to record the new item at the end of the dataset and return the new dataset to Prostar core. After clicking, the screen is not updated: all the inputs keep their current value and the ui is kept to the last step.
* A process module takes as parameter one dataset of 'n' items returns a dataset with N=(n+1) items,


## General logics for processes and datasets
...
* Each of data processing modules and dataset can be seen as a list of respectively P and N items. Let i,p be respectively the indices of respectively the current item in the dataset list and the current item in the process list of the workflow.
...
* After each run of a process p, one have i = p + 1 (+ offset)
