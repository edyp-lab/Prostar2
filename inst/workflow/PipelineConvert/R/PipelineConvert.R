...
#' @examples
#' if (interactive()){
#' source("~/GitHub/Prostar2/inst/extdata/workflow/PipelineConvert/R/PipelineConvert.R")
#' path <- system.file('extdata/workflow/PipelineConvert', package = 'Prostar2')
#' shiny::runApp(MagellanNTK::workflowApp("PipelineConvert"))
#' }
#' 
#' @name PipelineConvert
#' 
#' @example inst/workflow/PipelineConvert/examples/example_PipelineConvert.R
#' 
...
#' @rdname PipelineConvert
#' @export
#' 
PipelineConvert_conf <- function(){
  MagellanNTK::Config(
    mode = 'pipeline',
    fullname = 'PipelineConvert',
    steps = c('Convert'),
    mandatory = c(FALSE)
  )
}
...
#' @export
#' 
PipelineConvert_ui <- function(id){
  ns <- NS(id)
}
...
#' @export
#'
PipelineConvert_server <- function(id,
