...
#' 
#' In this example, `PipelinePeptideSC_Aggregation_ui()` and `PipelinePeptideSC_Aggregation_server()` define
...
#' shiny::runApp(proc_workflowApp("PipelinePeptideSC_Aggregation", path, dataIn = Exp2_R100_pept))
#' }
#' 
...
#' @export
#' 
PipelinePeptideSC_Aggregation_conf <- function(){
  MagellanNTK::Config(
    fullname = 'PipelinePeptideSC_Aggregation',
    mode = 'process',
    steps = c('Aggregation'),
    mandatory = c(TRUE)
  )
...
#' @export
#'
PipelinePeptideSC_Aggregation_ui <- function(id){
  ns <- NS(id)
}
...
#' @export
#' 
PipelinePeptideSC_Aggregation_server <- function(id,
