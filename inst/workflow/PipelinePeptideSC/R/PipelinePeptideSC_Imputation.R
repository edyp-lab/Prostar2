...
#' 
#' In this example, `PipelinePeptideSC_Imputation_ui()` and `PipelinePeptideSC_Imputation_server()` define
#' the code for the process `PipelinePeptideSC_Imputation` which is part of the pipeline called `PipelinePeptideSC`.
#' 
#' @examples
#' if (interactive()){
#' library(MagellanNTK)
...
#' shiny::runApp(proc_workflowApp("PipelinePeptideSC_Imputation", path, dataIn = Exp2_R100_pept))
#' }
#' 
...
#' @export
#' 
PipelinePeptideSC_Imputation_conf <- function(){
  MagellanNTK::Config(
    fullname = 'PipelinePeptideSC_Imputation',
    mode = 'process',
    steps = c('Imputation'),
    mandatory = c(FALSE)
  )
...
#' @export
#'
PipelinePeptideSC_Imputation_ui <- function(id){
  ns <- NS(id)
}
...
#' @export
#' 
PipelinePeptideSC_Imputation_server <- function(id,
