...
The central area of the interface displays multiple graphs. The graphs on the top shows the intensity distributions for each sample. For the left one, it is possible to hide a specified sample from the graph by clicking on its name in the legend. The right one can be switched from violin plot to a boxplot by clicking on the button above the graph.
The graph on the bottom provides a way to visually compare the quantitative data before and after normalization. This plot shows the influence of the normalization method and only appears once the data is normalized. 
